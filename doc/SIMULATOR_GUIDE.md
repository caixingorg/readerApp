# 📱 模拟器文件导入指南

## 🎯 问题说明

在模拟器中使用 Document Picker 时，可能看不到文件或无法选择文件。这是因为：

1. **iOS 模拟器**：文件系统与真机不同，需要先添加文件到模拟器
2. **Android 模拟器**：某些文件类型可能被过滤
3. **权限限制**：模拟器的沙盒环境比真机更严格

## 📋 解决方案

### iOS 模拟器

#### 方法 1: 拖拽文件（推荐）

1. **准备测试文件**
    - 在 Mac 上准备好 `.txt` 或 `.epub` 文件
    - 例如：创建一个简单的 `test.txt` 文件

2. **添加文件到模拟器**

    ```bash
    # 打开模拟器后，直接将文件拖拽到模拟器窗口
    # 文件会被保存到 Files app（文件应用）中
    ```

3. **使用 Files App**
    - 在模拟器中打开 **Files**（文件）应用
    - 点击 "On My iPhone" 或 "On My iPad"
    - 确认文件已经出现在那里

4. **在应用中导入**
    - 打开 Reader App
    - 点击"导入"
    - 在文件选择器中选择 "On My iPhone/iPad"
    - 选择刚才拖拽的文件

#### 方法 2: 使用 Safari 下载

1. 在模拟器中打开 Safari
2. 访问任何可以下载 epub/txt 文件的网站
3. 下载文件到 iCloud Drive 或 "On My iPhone"
4. 在 Reader App 中导入

#### 方法 3: 使用 iCloud Drive

1. 在 Mac 上将文件放到 iCloud Drive
2. 等待同步到模拟器
3. 在 Files app 中的 iCloud Drive 找到文件
4. 在 Reader App 中导入

### Android 模拟器

#### 方法 1: 通过 adb 推送文件

```bash
# 找到测试文件路径
# 推送到模拟器的 Downloads 目录
adb push /path/to/your/test.txt /sdcard/Download/

# 或者推送到 Documents
adb push /path/to/your/book.epub /sdcard/Documents/
```

#### 方法 2: 使用 Android Studio Device File Explorer

1. 打开 Android Studio
2. 点击 **View** → **Tool Windows** → **Device File Explorer**
3. 导航到 `/sdcard/Download/` 或 `/sdcard/Documents/`
4. 右键点击文件夹，选择 **Upload**
5. 上传你的测试文件

#### 方法 3: 在模拟器中下载

1. 在模拟器中打开 Chrome 浏览器
2. 下载测试文件
3. 文件会保存到 Downloads 文件夹
4. 在 Reader App 中导入时选择 Downloads

## 🔧 代码已优化

我已经调整了文件类型配置，使其在模拟器中更宽松：

```typescript
ios: {
    // 使用 'public.data' 和 'public.item' 以显示更多文件
    type: ['public.data', 'public.text', 'org.idpf.epub-container', 'public.item'],
    copyToCacheDirectory: true,
    multiple: false,
}

android: {
    // 使用通配符显示所有文件
    type: ['*/*'],
    copyToCacheDirectory: true,
    multiple: false,
}
```

## 📝 快速测试步骤

### iOS 模拟器完整流程

```bash
# 1. 创建测试文件
echo "这是一本测试书籍的内容。" > test.txt

# 2. 启动应用
npm run ios

# 3. 将 test.txt 拖拽到模拟器窗口

# 4. 在模拟器中：
#    - 打开 Files app，确认文件在 "On My iPhone"
#    - 打开 Reader App
#    - 点击"导入"
#    - 选择 test.txt
```

### Android 模拟器完整流程

```bash
# 1. 创建测试文件
echo "这是一本测试书籍的内容。" > test.txt

# 2. 启动应用
npm run android

# 3. 推送文件到模拟器
adb push test.txt /sdcard/Download/

# 4. 在 Reader App 中点击"导入"，选择 Downloads 中的文件
```

## 🎨 测试文件示例

### 简单 TXT 文件

```bash
cat > 测试小说.txt << 'EOF'
第一章 开始

这是第一章的内容。
故事从一个平凡的早晨开始。

第二章 冒险

主角踏上了未知的旅程。
EOF
```

### 最小 EPUB 文件

如果需要测试 EPUB，可以从这些地方下载：

- [Gutenberg.org](https://www.gutenberg.org/) - 免费电子书
- [FeedBooks](https://www.feedbooks.com/publicdomain) - 公共领域书籍

## ⚠️ 常见问题

### Q1: 拖拽文件后在 Files app 中找不到？

**A**: 确保拖拽到模拟器的主屏幕，而不是应用内部。文件会自动保存到 Files app 的 "On My iPhone/iPad" 位置。

### Q2: Android 模拟器 adb 命令不可用？

**A**: 确保已添加 Android SDK 到 PATH：

```bash
export ANDROID_HOME=$HOME/Library/Android/sdk
export PATH=$PATH:$ANDROID_HOME/platform-tools
```

### Q3: 文件选择器打开后是空的？

**A**:

- iOS: 检查 Files app，确保文件在 "On My iPhone"
- Android: 使用 `type: ['*/*']` 显示所有文件
- 尝试重启应用

### Q4: 导入后提示"不支持的文件格式"？

**A**: 确保文件扩展名正确：

```bash
# 正确的文件名
✅ test.txt
✅ book.epub

# 可能有问题的文件名
❌ test (没有扩展名)
❌ test.pdf (不支持的格式)
```

## 🚀 真机测试（强烈推荐）

模拟器始终有限制，**强烈建议使用真机测试**：

### iOS 真机

```bash
# 连接 iPhone 到 Mac
npm run ios -- --device "Your iPhone Name"
```

### Android 真机

```bash
# 启用 USB 调试后连接设备
npm run android
```

真机上的 Document Picker 工作方式：

- ✅ 可以访问所有文件应用（Files, iCloud, Google Drive, Dropbox 等）
- ✅ 可以直接从其他应用分享文件
- ✅ 权限管理更符合实际使用场景
- ✅ 性能更好，无卡顿

## 📊 对比总结

| 特性       | iOS 模拟器 | Android 模拟器 | 真机        |
| ---------- | ---------- | -------------- | ----------- |
| 文件访问   | 受限 ⚠️    | 受限 ⚠️        | 完整 ✅     |
| 性能       | 一般       | 一般           | 最佳 ✅     |
| 文件添加   | 需手动拖拽 | 需 adb 推送    | 直接访问 ✅ |
| 云存储     | 有限支持   | 有限支持       | 完整支持 ✅ |
| 测试准确性 | 中等       | 中等           | 最高 ✅     |

## 💡 开发建议

1. **开发阶段**：使用模拟器 + 手动添加测试文件
2. **测试阶段**：切换到真机测试
3. **调试时**：查看详细日志

    ```bash
    # iOS
    npx react-native log-ios

    # Android
    npx react-native log-android
    ```

4. **准备多个测试文件**：
    - 小文件（< 1MB）：快速测试
    - 中文件名：测试文件名处理
    - 大文件（> 10MB）：测试性能

## 🔍 调试技巧

如果仍然看不到文件，添加调试日志：

```typescript
// 在 handleImport 函数开始处
console.log('[DEBUG] Platform:', Platform.OS);
console.log('[DEBUG] Picker Config:', JSON.stringify(pickerConfig));

// 在文件选择后
console.log('[DEBUG] Selected file URI:', file.uri);
console.log('[DEBUG] File exists:', await FileSystem.getInfoAsync(file.uri));
```

祝测试顺利！🎉
